...
class TheGraphCluster extends Construct {
  ...
  constructor(scope, id, props) {
    ...
    if (props.allowedSG) {
      // Allow connections from the specified Security Group
      const allowedSecurityGroup = SecurityGroup.fromSecurityGroupId(
        this,
        'AllowedSG',
        props.allowedSG
      );
      ...
      nodeClientLaunchTemplate.connections.allowFrom(
        ...
        Port.tcp(80),
        'graph queries from allowed SG'
      );
      nodeClientLaunchTemplate.connections.allowFrom(
        allowedSecurityGroup,
        Port.tcp(8020),
        'graph deployment from allowed SG'
      );
      nodeClientLaunchTemplate.connections.allowFrom(
        allowedSecurityGroup,
        Port.tcp(8030),
        'graph status queries from allowed SG'
      );
      nodeClientLaunchTemplate.connections.allowFrom(
        allowedSecurityGroup,
        Port.tcp(5001),
        'IPFS deployment from allowed SG'
      );
    }
    ...
    if (props.allowedIP) {
      ...
      nodeClientLaunchTemplate.connections.allowFrom(
        ...
        Port.tcp(80),
        'graph queries from allowed IP'
      );
      nodeClientLaunchTemplate.connections.allowFrom(
        Peer.ipv4(`${props.allowedIP}/32`),
        Port.tcp(8020),
        'graph deployment from allowed IP'
      );
      nodeClientLaunchTemplate.connections.allowFrom(
        Peer.ipv4(`${props.allowedIP}/32`),
        Port.tcp(8030),
        'graph status queries from allowed IP'
      );
      nodeClientLaunchTemplate.connections.allowFrom(
        Peer.ipv4(`${props.allowedIP}/32`),
        Port.tcp(5001),
        'IPFS deployment from allowed IP'
      );
    }

    if (!props.allowedSG && !props.allowedIP) {
      throw new Error('Either allowedSG or allowedIP must be set.');
    }
